...
class AutoNav(Node):
    ...
    def isValid(self, row, col):
        ...
        return 0 <= row < rowsize and 0 <= col < colsize and self.cur_map[row][col] < 20 and self.cur_map[row][col] != -1
    ...
    def calculate_cw_rotation_angle(self, F, T):
        front_length = T[0] - F[0]
        side_length = T[1] - F[1]
        if side_length == 0:
            return 90
        return np.degrees(np.arctan(front_length / side_length))
    ...
    def occ_callback(self, msg):
        ...
        grid_x = round((cur_pos.x - self.map_origin.x) / self.map_res) # column in numpy 
        grid_y = round(((cur_pos.y - self.map_origin.y) / self.map_res)) # row in numpy
    ...
    def find_target(self):
        ...
        self.get_logger().info('These are the list of visited points')
        for x, y in self.visited_points:
            self.get_logger().info(f'Row Col: {x}, {y}')
        ...
        for row, col in points[1:]:
            ...
            while distance > localization_tolerance ** 2:
                ...
                if first_time:
                    ...
                    map_angle = 90 + self.calculate_cw_rotation_angle((self.currow, self.curcol), (row, col))
                    if col <= self.curcol:
                        map_angle += 180
